...
## 1. Browse (requires login)

С 2026-09-24 search и grouping требуют авторизации — см. `03_FRONTEND.md` §9 (нужны метрики по
активным пользователям/клиентам, для этого каждый search-запрос должен быть привязан к
`user_id`). Раньше сценарий назывался "Anonymous browse" и не требовал логина на шагах 1-6.

1. Пользователь открывает сайт; если не залогинен — редирект на login.
2. Выбирает make/model.
3. Получает существующие listings.
4. Видит freshness.
5. Видит market price.
6. Может открыть listing.
7. Может сохранить поиск (уже залогинен).
