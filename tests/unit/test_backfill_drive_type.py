...
from app.scraping.backfill_drive_type import _backfill
...
async def test_backfill_sets_drive_type_only_where_missing(session, monkeypatch):
    monkeypatch.setitem(registry.SOURCE_REGISTRY, "polovniautomobili", _StubAdapter)
    source = await seed_source(session)

    missing = make_listing(source.id, external_id="1", drive_type=None)
    already_set = make_listing(source.id, external_id="2", drive_type="front")
    session.add(missing)
...
async def test_backfill_skips_blocked_listings_without_aborting(session, monkeypatch):
    ...
    session.add(make_listing(source.id, external_id="blocked", drive_type=None))
    session.add(make_listing(source.id, external_id="no-value", drive_type=None))
    await session.commit()
    ...
    assert refreshed["blocked"].drive_type is None
    assert refreshed["no-value"].drive_type is None
