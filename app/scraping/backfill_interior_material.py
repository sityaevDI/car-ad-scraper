"""One-off backfill for `interior_material` on listings scraped before that field existed.
...
*search-result* raw JSON for those — which doesn't reliably carry interior_material (only
sometimes, buried in a premium listing's `featuredInfo`) — so there's no way to backfill them
from data already in Postgres.

Usage:
    python -m app.scraping.backfill_interior_material [--source polovniautomobili] [--limit N]
"""
...
async def _backfill(session: AsyncSession, source_code: str, limit: int | None) -> None:
    ...
    stmt = select(Listing).where(
        ...
        Listing.interior_material.is_(None),
    )
    if limit is not None:
        stmt = stmt.limit(limit)
    listings = (await session.execute(stmt)).scalars().all()
    print(f"{len(listings)} listing(s) missing interior_material")
