"""One-off backfill for `drive_type` on listings scraped before that field existed.
...
*search-result* raw JSON for those — which doesn't reliably carry drive type (only sometimes,
buried in a premium listing's `featuredInfo.drive`) — so there's no way to backfill them from data
already in Postgres. Mirrors backfill_interior_material.py/backfill_air_condition.py.

Usage:
    python -m app.scraping.backfill_drive_type [--source polovniautomobili] [--limit N]
"""
...
async def _backfill(session: AsyncSession, source_code: str, limit: int | None) -> None:
    ...
    stmt = select(Listing).where(
        ...
        Listing.drive_type.is_(None),
    )
    if limit is not None:
        stmt = stmt.limit(limit)
    listings = (await session.execute(stmt)).scalars().all()
    print(f"{len(listings)} listing(s) missing drive_type")
    ...
    for i, listing in enumerate(listings, start=1):
        ...
        if detail.air_condition and not listing.air_condition:
